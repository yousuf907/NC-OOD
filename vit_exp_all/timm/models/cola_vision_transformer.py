...
from .layers import PatchEmbed, DropPath, trunc_normal_, lecun_normal_, ACT_Mlp
from .registry import register_model
from .cola_layer import ColaLayer ## added
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        rf=4
        self.fc1 = ColaLayer(in_features, hidden_features, rank= min(in_features, hidden_features) // rf, 
            bias=True, lr_act=True, lr_act_type=act_layer)
        self.fc2 = ColaLayer(hidden_features, out_features, rank= min(hidden_features, out_features) // rf, 
            bias=True, lr_act=True, lr_act_type=act_layer)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        rf=4
        self.qkv = ColaLayer(dim, dim * 3, rank=dim // rf, bias=qkv_bias, lr_act=True, lr_act_type=nn.GELU)

        self.attn_drop = nn.Dropout(attn_drop)

        #self.proj = nn.Linear(dim, dim)
        self.proj = ColaLayer(dim, dim, rank=dim // rf, bias=True, lr_act=True, lr_act_type=nn.GELU)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, output_dim=0, hidden_mlp=0,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True, representation_size=None, 
                 distilled=False, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, 
                 norm_layer=None, act_layer=None, weight_init='', args=None):
        """
        Args:
        ...
        if output_dim == 0:
            ...
            ## ETF (NC) >> Two Layers (192 --> 768 --> 192)
            self.projection_head = nn.Sequential(
                nn.Linear(in_features=self.num_features, out_features=hidden_mlp, bias=False), ## 192 > 768
                nn.ReLU(inplace=True),
                nn.Linear(in_features=hidden_mlp, out_features=output_dim, bias=False), # 768 > 192
            )
    ...
    def _intermediate_layers(
        ...
        ## Consider the last encoder layer only
        x=self.blocks(x) # N x 196 x 192
        x = self.norm(x) 
        x = x.mean(dim = 1) # N x 192
        ...
        if self.projection_head is not None:
            ...
            ## L2 normalize >> NC
            x = F.normalize(x, eps=1e-8, p=2, dim=-1)
            outputs.append(x)
    ...
    def get_intermediate_layers(
        ...
        #if norm: << CHECK THIS FOR OOD DETECTION >>
    ...
    def forward(self, x, embed=False):
        ...
        if self.projection_head is not None:
            ...
            ## L2 normalization to induce Neural Collapse
            #xe = F.normalize(xe0, eps=1e-8, p=2, dim=-1)
            xe = xe0

        x_out = self.head(xe)

        if embed:
            #xe0 = F.normalize(xe0, eps=1e-8, p=2, dim=-1)
            return xe0, xe, x_out
        else:
            return x_out
