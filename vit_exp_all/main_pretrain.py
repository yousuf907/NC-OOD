...
def get_args_parser():
    ...
    parser.add_argument('--alpha', default=0.05, type=float, help='entropy reg loss weight')
    return parser
...
def main(args):
    ...
    print('\nSize of train loader', len(dataset_train))
    print('\nSize of val loader', len(dataset_val))
    ...
    print('\nNumber of trainable params (in Millions):', n_parameters / 1e6)

    linear_scaled_lr = args.lr * args.batch_size * utils.get_world_size() / 512.0
    print("Scaled LR:", linear_scaled_lr)
    ...
    print("\nUsing label smoothing of ", args.smoothing)
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=args.smoothing).cuda()

    if args.alpha > 0:
        print("Using Entropy Reg Loss with weight ", args.alpha)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
                model, criterion, aux_criterion, data_loader_train,
...
if __name__ == '__main__':
    ...
    if args.alpha > 0:
        from engine import train_one_epoch, evaluate, visualize # proposed model
    else:
        from engine_base import train_one_epoch, evaluate, visualize # baseline model
