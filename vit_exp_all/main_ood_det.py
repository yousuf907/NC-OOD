...
from engine_ood_det import train_one_epoch, evaluate, ood_evaluate, evaluate_ckpt, visualize
...
def main(args):
    ...
    #### ~~~~ OOD Datasets ~~~~~~~ ####
    ood_datasets = ['IMNETR', 'CIFAR', 'Flower102', 'CUB200', 'Aircraft', 'Pet', 'Stl']
    ## NINCO odd detection is in another script 'main_evalnc.py' (run eval_nc_baseline.sh & eval_nc_ours.sh)

    for d in ood_datasets:
        args.data_set = d
        print('Processing OOD Dataset:', d)

        ## Load Datasets
        dataset_train2, ood_classes = build_dataset(is_train=True, args=args)
        dataset_val2, _ = build_dataset(is_train=False, args=args)
        print("Number of classes in the OOD dataset:", ood_classes)

        if True:
            num_tasks = utils.get_world_size()
            global_rank = utils.get_rank()
            if args.repeated_aug:
                sampler_train2 = RASampler(
            ...
            if args.dist_eval:
                if len(dataset_val2) % num_tasks != 0:
                    print('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
                        'This will slightly alter validation results as extra duplicate entries are added to achieve '
                        'equal num of samples per-process.')
                sampler_val2 = torch.utils.data.DistributedSampler(
                    dataset_val2, num_replicas=num_tasks, rank=global_rank, shuffle=False)
            else:
        ...
        print('\nSize of train loader (OOD)', len(dataset_train2))
        print('\nSize of val loader (OOD)', len(dataset_val2))
        ...
        ## NECO > engine_ood_neco
        #fpr, auroc = ood_evaluate(data_loader_train, data_loader_val, data_loader_val2, model, linear_classifier, device, epoch, tf_writer=tf_writer, args=args)

        print('\nOOD Set :', args.data_set)
        print("\nFPR :", 100*fpr)
        print("\nAUROC :", 100*auroc)
