...
def build_dataset(is_train, args):
    ...
    if args.data_set == 'CIFAR':
        ...
        dataset = Flowers102(args.data_path, split=split, download=True, transform=transform)
        nb_classes = 102

    elif args.data_set == 'CUB200':
        args.data_path='/home/yousuf/cub200'
        #dataset = CUB200(args.data_path, train=is_train, download=True, transform=transform).data
        root = os.path.join(args.data_path, 'train' if is_train else 'val')
        dataset = datasets.ImageFolder(root, transform=transform)
        nb_classes = 200

    elif args.data_set == 'Scene67':
        dataset = Scene67(args.data_path, train=is_train, download=True, transform=transform).data
        nb_classes = 67
        ...
        if is_train:
            ...
            split = 'test'
        dataset = datasets.FGVCAircraft(args.data_path, split=split, download=True, transform=transform)
        nb_classes = 100
    ...
    #    nb_classes = 67
        ...
        if is_train:
            ...
            split = 'test'
        dataset = datasets.OxfordIIITPet(args.data_path, split=split, download=True, transform=transform)
        nb_classes = 37
