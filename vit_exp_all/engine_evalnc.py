    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.no_grad():
            ...
            for each_layer in intermediate_output:
                output.append(each_layer.float())
...
## NC Analysis
def nc_analysis(model, num_classes, loader, args, proj=True):
    model.eval()
    device='cuda'
    C = num_classes
    epoch=1
    ...
    with torch.no_grad():
        ...
        for computation in ['Mean','Cov']:
            ...
            for batch_idx, (data, target) in enumerate(loader, start=1):
                ...
                if proj==True: # embedding from projector
                    _, h, output = model(data, embed=True) # encoder_emb, proj_emb, pred
                else: # embedding from encoder
                    h, _, output = model(data, embed=True) # encoder_emb, proj_emb, pred
...
@torch.no_grad()
def ood_evaluate(data_loader, data_loader2, model, linear_classifier, device, epoch, tf_writer=None, args=None):
    print("\nOOD detection begings")
    print("\nNumber of ID class count:", args.nb_classes)
    criterion = torch.nn.CrossEntropyLoss()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    ...
    ## ID logits
    logits = torch.zeros((args.num_layers, len(data_loader.dataset), args.nb_classes), dtype=torch.float64) # dim: L x N x C
    start_ix = 0
    ...
    for images, target in metric_logger.log_every(data_loader, 50, header):
        ...
        for each_output, each_classifier in zip(output, linear_classifier):
            ...
            ##
            logits[i-1, start_ix:end_ix] = each_output
    ...
    ## OOD Logits
    logits2 = torch.zeros((args.num_layers, len(data_loader2.dataset), args.nb_classes), dtype=torch.float64) # dim: L x N x C
    start_ix = 0

    for images, target in metric_logger.log_every(data_loader2, 50, header):
        ...
        for each_output, each_classifier in zip(output, linear_classifier):
            ...
            ##
            logits2[i-1, start_ix:end_ix] = each_output
    ...
    fpr_arr=[]
    auroc_arr=[]
    for m in range(args.num_layers):
        ## energy-scores
        known = torch.logsumexp(logits[m].data.cpu(), dim=1).numpy()
        novel = torch.logsumexp(logits2[m].data.cpu(), dim=1).numpy()
        ### OOD Detection
        fpr, auroc = compute_ood_det(known, novel, out_datasets="Dataset", method="Energy")
        fpr_arr = np.append(fpr_arr, fpr)
        auroc_arr = np.append(auroc_arr, auroc)

    return fpr_arr, auroc_arr
