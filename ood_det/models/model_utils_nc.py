...
from .vgg_etf import * # Baseline without ETF Projector or Entropy Reg
from .resnet_etf import * # Baseline without ETF Projector or Entropy Reg
...
def get_model(cfg):
    ...
    if cfg.arch == 'ResNet18':
        ...
        return resnet18(num_classes=cfg.num_cls, output_dim=512, hidden_mlp=2048)

    if cfg.arch == 'ResNet34':

        return resnet34(num_classes=cfg.num_cls, output_dim=512, hidden_mlp=2048)

    if cfg.arch == 'vgg17':

        return VGG("VGG17", class_num=cfg.num_cls, output_dim=512, hidden_mlp=2048)
